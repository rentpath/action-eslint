...
NPM_BIN=$(npm bin)/eslint

if [ "${INPUT_TSLINT}" == 'true' ]; then
  $NPM_BIN/tslint --version

  if [ "${INPUT_REPORTER}" == 'github-pr-review' ]; then
    # Use jq and github-pr-review reporter to format result to include link to rule page.
    $NPM_BIN/tslint --project ${INPUT_LINT_DIRS:-'.'} --config "${INPUTLINT_CONFIG:-tslint.json}" --format json \
    ...
    $NPM_BIN/tslint --format="stylish" --project ${INPUT_LINT_DIRS:-'.'} --config "${INPUT_LINT_CONFIG:-tslint.json}" \
      | reviewdog -f="tslint" -name="${INPUT_NAME:-tslint}" -reporter="${INPUT_REPORTER:-github-pr-check}" -level="${INPUT_LEVEL}"
  fi
else
  $NPM_BIN/eslint --version

  if [ "${INPUT_REPORTER}" == 'github-pr-review' ]; then
    # Use jq and github-pr-review reporter to format result to include link to rule page.
    $NPM_BIN/eslint ${INPUT_LINT_DIRS:-'.'} --ext="${INPUT_LINT_EXT}" --config "${INPUT_LINT_CONFIG}" -f json \
      | jq -r '.[] | {filePath: .filePath, messages: .messages[]} | "\(.filePath):\(.messages.line):\(.messages.column):\(.messages.message) [\(.messages.ruleId)](https://eslint.org/docs/rules/\(.messages.ruleId))"' \
      | reviewdog -efm="%f:%l:%c:%m" -name="${INPUT_NAME}:-eslint}" -reporter=github-pr-review -level="${INPUT_LEVEL}"
  else
    $NPM_BIN/eslint ${INPUT_LINT_DIRS:-'.'} --ext="${INPUT_LINT_EXT}" --config "${INPUT_LINT_CONFIG}" \
